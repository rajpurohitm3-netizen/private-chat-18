...
type ActiveView = "dashboard" | "chat" | "vault" | "calls" | "connections" | "settings" | "advanced";
...
export function UserDashboardView({ session, privateKey }: UserDashboardViewProps) {
  ...
  const [advancedSubView, setAdvancedSubView] = useState<"menu" | "cinema" | "music" | "memories">("menu");
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <>
            ...
            <motion.div initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed top-0 left-0 bottom-0 w-[80%] max-w-xs bg-[#050505] border-r border-white/5 z-[101] lg:hidden p-6 flex flex-col">
                ...
                {advancedSubView === "cinema" && (
                  <div className="p-6 sm:p-8 pb-32 lg:pb-12">
                    ...
                    <div className="mb-8">
                      ...
                      <p className="text-sm text-white/40">Watch movies together with friends</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {friendProfiles.map(p => (
